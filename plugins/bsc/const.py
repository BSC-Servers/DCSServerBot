...
RANK_CODES = {rank["code"]: level for level, rank in BSC_RANKS.items()}


def get_rank_for_xp(xp: int) -> tuple[int | None, dict | None]:
    for rank in reversed(list(BSC_RANKS.keys())):
        if xp >= BSC_RANKS[rank]["requiredXP"]:
            return rank, BSC_RANKS[rank]
    return None, None
